...
followers <- get_account_followers(id, limit = 400L) # récupération des followers. une limite doit être fixée pour que le script fonctionne et récupère tous les messages, la situer à la centaine supérieure par rapport au nombre de messages envoyés d'après le profil du compte
following <- get_account_following(id, limit = 400L) 
...
dataframe <- get_account_statuses(id, limit = 400L) # récupération des messages - une limite doit être fixée pour que le script fonctionne et récupère tous les messages, la situer à la centaine supérieure par rapport au nombre de messages envoyés d'après le profil du compte
toots <- get_account_statuses(id, exclude_reblogs = TRUE, limit = 400L) #récupération des messages moins les boosts (les messages d'autrui que nous avons reposés). Nécessaire pour déterminer par soustraction de l'ensemble des messages, les messages qui ont été boostés. 
...
Le bilan quantitatif est bien entendu présent dans cette étude et il peut être très rapidement remis à jour en exécutant le code sous-jacent à ce texte. 
Ce code peut d'ailleurs être récupéré sur Software Heritage pour servir à analyser l'activité de n'importe quel autre usager ou usagère d'un compte sur Mastodon. 
...
C'est notre défaut d'intérêt pour le numérique (ce dernier étant vu toujours comme un  moyen et jamais comme un objet d'intérêt en soi) ou bien notre cécité sur la manière dont le capitalisme de plateforme entremêle jusqu'à les rendre indissociables le numérique, le politique et l'économique qui font que nous avons le sentiment de naviguer dans les ténèbres. Attirés comme des papillons par la lumière de réseaux dont les propriétaires se considèrent eux-mêmes comme des Soleils, nous errons longuement autour de ce feu jusqu'à nous y brûler :
