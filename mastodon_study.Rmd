...
```{r}
library(rtoot)
id <- "112370075539544475"auth
get_account_followers(id)
get_account_following(id)
get_account_statuses(id, limit = 300L)
```
...
write_csv(df2, "SO_univrennes_toots.csv")# Now df2$content will have the text without HTML tags
```
